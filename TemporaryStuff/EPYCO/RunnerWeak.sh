...
do
  	echo "Running with $i threads..."
        ...
        # Adjust MPI command to handle oversubscription gracefully
        echo "With OMP off..." >> "$output_file"
        command="mpiexec --use-hwthread-cpus --oversubscribe --bind-to hwthread --map-by ppr:1:hwthread -np $i $progra>
        ...
        echo "With OMP on..." >> "$output_file"
        command2="mpiexec --use-hwthread-cpus -np $i $program_name $xl $yl $xr $yr $width_weak $height_weak $max_itera>
