...
for map in $map_values; do
	for np in $np_values; do
		...
		echo "...Benchmarking ${algorithm} with map=$map and np=$np..."
		mpirun -np $np -map-by $map --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 1 ${src_path}osu_bcast -x 1000 -i 1000 | tail -n 21 \
